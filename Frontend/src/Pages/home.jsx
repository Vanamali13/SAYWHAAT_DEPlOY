...
export default function Home() {
  ...
  return (
    <div className="min-h-screen">
        ...
        <div className="relative max-w-7xl mx-auto px-6 py-24 md:py-32">
          ...
          >
            <div className="inline-block">
              <Badge variant="outline" className="bg-zinc-900/80 text-white border-zinc-700 backdrop-blur-md px-6 py-2 text-sm font-medium shadow-lg hover:bg-zinc-800 transition-all">
              ...
              {isAuthenticated ? (
                <>
                  <Link to={createPageUrl("CreateDonation")}>
                    <Button size="lg" className="bg-white text-black hover:bg-zinc-200 shadow-2xl px-8 py-6 text-lg">
                      <Gift className="w-5 h-5 mr-2" />
                      Make a Donation
                    </Button>
                  </Link>
                  <Link to={createPageUrl("DonorDashboard")}>
                    <Button size="lg" variant="outline" className="border-zinc-700 text-zinc-300 hover:bg-zinc-800 px-8 py-6 text-lg backdrop-blur-sm">
                      View My Impact
                    </Button>
                  </Link>
                </>
                ...
                <Link to={createPageUrl("SignUp")}>
                  <Button
                    size="lg"
                    className="bg-white text-black hover:bg-zinc-200 shadow-2xl px-8 py-6 text-lg"
                  >
                    Get Started
                  </Button>
                </Link>
      ...
      <div className="max-w-7xl mx-auto px-6 -mt-16 relative z-10">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {[
            ...
            >
              <Card className="backdrop-blur-md bg-zinc-900 border-zinc-800 shadow-2xl h-full">
                <CardContent className="py-12 px-6 flex flex-col items-center justify-center h-full">
                  <stat.icon className="w-12 h-12 mb-6 text-white" />
                  <p className="text-4xl font-bold text-white mb-2">{stat.value}</p>
                  <p className="text-md text-zinc-400">{stat.label}</p>
                </CardContent>
              </Card>
            </motion.div>
          ))}
      ...
      <div className="max-w-7xl mx-auto px-6 py-24">
        ...
        <div className="grid md:grid-cols-3 gap-8">
          {features.map((feature, idx) => (
            ...
            >
              <Card className="h-full hover:shadow-2xl transition-all duration-300 backdrop-blur-sm bg-zinc-900 border-zinc-800">
                <CardHeader>
                  <div className="w-16 h-16 bg-zinc-800 rounded-2xl flex items-center justify-center mb-4 shadow-lg border border-zinc-700">
      ...
      {/* CTA Section */}
      <div className="bg-zinc-900 border-t border-zinc-800 text-white">
