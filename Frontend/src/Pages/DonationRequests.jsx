    ...
    const getStatusBadge = (status) => {
        switch (status) {
            case 'pending_approval':
                return <Badge variant="outline" className="bg-yellow-500/10 text-yellow-500 border-yellow-500/20 px-2 py-0.5 text-xs"><Clock className="w-3 h-3 mr-1" />Pending</Badge>;
            case 'rejected':
                return <Badge variant="outline" className="bg-red-500/10 text-red-500 border-red-500/20 px-2 py-0.5 text-xs"><X className="w-3 h-3 mr-1" />Rejected</Badge>;
            default:
                return <Badge variant="outline" className="bg-green-500/10 text-green-500 border-green-500/20 px-2 py-0.5 text-xs"><Check className="w-3 h-3 mr-1" />Accepted</Badge>;
        }
    };

    return (
        <div className="group p-4 border border-zinc-800 rounded-xl bg-zinc-900/40 hover:bg-zinc-900/80 transition-all duration-200 hover:border-zinc-700 shadow-sm relative overflow-hidden">
            {/* Header */}
            <div className="flex justify-between items-start mb-4 relative z-10">
                <div className="flex items-center gap-3 min-w-0 flex-1 mr-2">
                    ...
                    <div className="overflow-hidden min-w-0">
                        <h4 className="font-semibold text-zinc-200 truncate">{donation.donor.name}</h4>
            ...
            {/* Content */}
            <div className="space-y-3 bg-zinc-950/50 p-4 rounded-lg border border-zinc-800/50 relative z-10">
                <div className="flex items-center justify-between text-sm">
                    <span className="text-zinc-500 font-medium">Type</span>
                    <span className="text-zinc-200 font-semibold capitalize">{donation.donation_type}</span>
                </div>
                {donation.amount > 0 && (
                    <div className="flex items-center justify-between text-sm">
                        <span className="text-zinc-500 font-medium">Amount</span>
                        <span className="text-zinc-200 font-semibold flex items-center"><DollarSign className="w-3 h-3 mr-1" />{donation.amount}</span>
                    </div>
                )}
                {donation.items?.length > 0 && (
                    <div className="text-sm pt-2 border-t border-zinc-800/50 mt-2">
                        <span className="text-zinc-500 font-medium block mb-2 flex items-center gap-1"><Box className="w-3 h-3" /> Items</span>
                        <div className="flex flex-wrap gap-1.5">
                            {donation.items.map((item, idx) => (
                                <span key={idx} className="inline-flex items-center px-2.5 py-1 rounded-md bg-zinc-800/80 text-zinc-300 text-xs border border-zinc-700/50">
                                    {item.name}
            ...
            {/* Footer */}
            <div className="flex items-center justify-between mt-4 pt-3 border-t border-zinc-800/50 relative z-10">
                <p className="text-[10px] text-zinc-500 flex items-center gap-1.5">
                    <Calendar className="w-3 h-3" />
                    {format(new Date(donation.createdAt), 'MMM d, yyyy • h:mm a')}
                </p>
                {donation.donationId && (
                    <p className="text-[10px] font-mono text-zinc-600 bg-zinc-900 px-1.5 py-0.5 rounded border border-zinc-800">{donation.donationId}</p>
                )}
            </div>
        </div>
    );
...
const RequestCategory = ({ title, donations, onApprove, onReject, isLoadingMutations }) => {
    ...
    return (
        <Card className="bg-zinc-900/30 border-zinc-800 shadow-xl overflow-hidden backdrop-blur-sm h-full flex flex-col">
            <CardHeader className="border-b border-zinc-800 bg-zinc-900/50 py-4 px-5">
                <CardTitle className={`text-base font-semibold ${getHeaderColor()} flex items-center gap-2.5`}>
                    {getHeaderIcon()}
                    {title}
                    <Badge variant="secondary" className="ml-auto bg-zinc-800 text-zinc-400 border-zinc-700 font-mono text-xs">{donations.length}</Badge>
                </CardTitle>
            </CardHeader>
                ...
                <div className="flex-1 overflow-y-auto p-4 space-y-4 custom-scrollbar">
                    {donations.length > 0 ? (
                        donations.map(donation => (
                            <div key={donation._id} className="relative group/card">
                                ...
                                {title === 'Pending Approval' && (
                                    <div className="absolute inset-x-0 bottom-0 p-4 bg-gradient-to-t from-zinc-950 via-zinc-950/90 to-transparent opacity-0 group-hover/card:opacity-100 transition-opacity duration-200 flex items-end justify-center gap-2 pt-12 rounded-b-xl">
                        ...
                        <div className="h-full flex flex-col items-center justify-center text-zinc-500 py-12 px-4 text-center">
                            ...
                            <p className="text-xs text-zinc-600 mt-1">New requests will appear here.</p>
                        </div>
                    )}
                </div>
            </CardContent>
...
export default function DonationRequests() {
    ...
    return (
        <div className="min-h-screen bg-zinc-950 text-zinc-100 flex flex-col h-screen overflow-hidden">
            <div className="flex-none p-6 lg:p-8 border-b border-zinc-900 bg-zinc-950/50 backdrop-blur-xl z-20">
                <div className="max-w-[1800px] mx-auto flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                    <div>
                        <h1 className="text-2xl md:text-3xl font-bold text-white tracking-tight">Donation Requests</h1>
                        <p className="text-zinc-400 mt-1 text-sm">Manage and track all donation statuses</p>
                    </div>
