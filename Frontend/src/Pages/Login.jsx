...
export default function Login() {
  ...
  const handleSendOtp = async () => {
    ...
    setError("");
    try {
      await apiClient.post('/users/send-otp', { identifier: form.email });
      setOtpSent(true);
    } catch (err) {
        ...
        <CardContent>
          <div className="flex justify-center mb-6">
            <div className="bg-zinc-100 dark:bg-zinc-800 p-1 rounded-lg inline-flex">
              <button
                type="button"
                className={`px-4 py-2 text-sm font-medium rounded-md transition-all ${!isOtpLogin
                  ? "bg-white dark:bg-zinc-900 text-zinc-900 dark:text-white shadow-sm"
                  : "text-zinc-500 dark:text-zinc-400 hover:text-zinc-900 dark:hover:text-white"
                  }`}
                onClick={() => { setIsOtpLogin(false); setError(""); setOtpSent(false); }}
              >
                Password
              </button>
              <button
          ...
          <form onSubmit={isOtpLogin ? handleOtpSubmit : handleSubmit} className="space-y-6">
            <div className="space-y-2">
              <Label htmlFor="email" className="text-zinc-500 dark:text-zinc-400">
                {isOtpLogin ? "Email Address" : "Email Address or Phone Number"}
              </Label>
              <Input
                id="email"
                type={isOtpLogin ? "email" : "text"}
                name="email"
                placeholder={isOtpLogin ? "you@example.com" : "you@example.com or +1234567890"}
                value={form.email}
            ...
            {isOtpLogin && otpSent && (
              <div className="space-y-2">
                <Label htmlFor="otp" className="text-zinc-500 dark:text-zinc-400">Enter Email OTP</Label>
                <Input
                  id="otp"
                  type="text"
                  name="otp"
            ...
            {isOtpLogin ? (
              !otpSent ? (
                <Button type="button" onClick={handleSendOtp} className="w-full font-semibold bg-zinc-900 dark:bg-white text-white dark:text-black hover:bg-zinc-800 dark:hover:bg-zinc-200 py-3" disabled={loading}>
