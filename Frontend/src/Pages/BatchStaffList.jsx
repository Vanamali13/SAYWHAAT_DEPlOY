...
export default function BatchStaffList() {
  ...
  if (isError) {
    return <div className="min-h-screen p-6 bg-zinc-950 text-red-500 flex items-center justify-center">Failed to load batch staff.</div>;
  }
    ...
    <div className="min-h-screen p-6 lg:p-8 bg-zinc-950 text-zinc-100">
      <div className="max-w-5xl mx-auto space-y-8">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 pb-6 border-b border-zinc-900">
          <div>
            <h1 className="text-3xl font-bold text-white tracking-tight">Batch Staff List</h1>
            <p className="text-zinc-400 mt-2">Manage and view all registered batch staff members</p>
          </div>
        ...
        <div className="flex flex-col sm:flex-row gap-4">
          <div className="relative flex-1">
            ...
            <Input
              ...
              className="pl-10 bg-zinc-900/50 border-zinc-800 text-zinc-200 placeholder:text-zinc-600 focus:ring-blue-500/20 focus:border-blue-500/50"
            />
          </div>
          <select
            value={filter}
            onChange={e => setFilter(e.target.value)}
            className="h-10 px-3 rounded-md bg-zinc-900/50 border border-zinc-800 text-zinc-200 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500/50 min-w-[120px]"
          >
            ...
            <table className="min-w-full divide-y divide-zinc-800">
              ...
              <tbody className="divide-y divide-zinc-800/50">
                {filteredStaff.length === 0 ? (
                  ...
                  <tr key={staff._id} className="group hover:bg-zinc-900/80 transition-colors">
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="flex items-center gap-3">
                        ...
                        <span className="text-zinc-200 font-medium">{staff.name}</span>
                      </div>
                    </td>
