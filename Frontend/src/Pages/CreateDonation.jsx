...
export default function CreateDonation() {
  ...
  const [formData, setFormData] = useState({
    ...
    delivery_notes: "",
    scheduled_delivery: "",
    address: user?.address || "", // Prefill from user profile
  ...
  const handleSubmit = (e) => {
    ...
    if (formData.donation_type !== 'money') {
      ...
      // Scheduled delivery is now estimated by the system, no validation needed here.
    }
              ...
              <div className="grid md:grid-cols-2 gap-6">
                {formData.donation_type !== 'money' && (
                  <>
                    ...
                    <div className="space-y-2">
                      <Label className="text-zinc-500 dark:text-zinc-400">Estimated Delivery</Label>
                      <div className="p-3 bg-zinc-100 dark:bg-zinc-800 rounded-md text-zinc-600 dark:text-zinc-400 text-sm border border-zinc-200 dark:border-zinc-700">
                        Date will be assigned upon approval.
                      </div>
                    </div>
                    <div className="space-y-2 md:col-span-2">
                      <Label htmlFor="wish_message" className="text-zinc-500 dark:text-zinc-400">Message/Wish for Receiver (for NFC Tag)</Label>
                      <Textarea
                        id="wish_message"
                        placeholder="A short wish or message to be imprinted on the NFC tag..."
                        value={formData.wish_message || ''}
                        onChange={(e) => setFormData(prev => ({ ...prev, wish_message: e.target.value }))}
                        className="bg-zinc-50 dark:bg-zinc-950 border-zinc-200 dark:border-zinc-800 text-zinc-900 dark:text-white"
                      />
                      <p className="text-xs text-zinc-400">This message along with your name will be digitally imprinted on the attached NFC tag.</p>
                    </div>
                  </>
                )}
              </div>
